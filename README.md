...
1. Press the "Use this template" button at the top of this repo to create a new repo with the contents of this og-image.
...
```bash
php artisan vendor:publish --tag="laravel-og-image-migrations"
php artisan migrate
```
...
```bash
php artisan vendor:publish --tag="laravel-og-image-config"
```
...
```bash
php artisan vendor:publish --tag="laravel-og-image-views"
```

## Usage

```php
$:ogImage = new Spatie\OgImage();
echo $:ogImage->echoPhrase('Hello, Spatie!');
```
